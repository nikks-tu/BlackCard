...
import com.techuva.blackcard.contusfly.frescoimagezoomable.ZoomableDraweeView;
...
public class FullImageView extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(isAdmin){
            Glide.with(this).load(R.drawable.ic_logo_icon).error(getApplicationContext().getResources().getDrawable(R.drawable.placeholder_image))
                    .fitCenter()
                    //.bitmapTransform(new RoundedCornersTransformation(this, 5, 5, 0, RoundedCornersTransformation.CornerType.ALL))
                    .diskCacheStrategy(DiskCacheStrategy.RESOURCE)
                    .dontAnimate()
                    .placeholder(getApplicationContext().getResources().getDrawable(R.drawable.placeholder_image)).into(imgProfile);
        }else {
            Glide.with(this).load(campaignLogo).error(getApplicationContext().getResources().getDrawable(R.drawable.placeholder_image)).fitCenter()
                //    .bitmapTransform(new RoundedCornersTransformation(this, 5, 5, 0, RoundedCornersTransformation.CornerType.ALL))
                    .diskCacheStrategy(DiskCacheStrategy.RESOURCE)
                    .dontAnimate()
                    .placeholder(getApplicationContext().getResources().getDrawable(R.drawable.placeholder_image)).into(imgProfile);
