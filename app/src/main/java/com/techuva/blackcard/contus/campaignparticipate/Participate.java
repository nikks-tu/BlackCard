package com.techuva.blackcard.contus.campaignparticipate;
...
import com.techuva.blackcard.contus.responsemodel.UserPollResponseModel;
import com.techuva.blackcard.contus.restclient.CampaignPollRestClient;
import com.techuva.blackcard.contus.views.EndLessListView;
...
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_CATEGORY;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_DATE;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_ID;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_LOGO;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_NAME;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_POLL_COMMENTS_COUNT;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_POLL_COMMENTS_SIZE;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_POLL_ID_ANSWER_ARRAY;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_POLL_ID_ANSWER_SELECTED_ARRAY;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_POLL_ID_ANSWER_SIZE;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_POLL_ID_SELECTED_SIZE;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_POLL_LIKES_USER_ARRAY;
import static com.techuva.blackcard.contus.app.Constants.CAMPAIGN_POLL_LIKES_USER_SIZE;
import static com.techuva.blackcard.contus.app.Constants.CAMPIGN_POLL_LIKES_COUNT_ARRAY;
import static com.techuva.blackcard.contus.app.Constants.CAMPIGN_POLL_LIKES_COUNT_SIZE;
import static com.techuva.blackcard.contus.app.Constants.CAMPIGN_POLL_PARTICIPATE_COUNT_ARRAY;
import static com.techuva.blackcard.contus.app.Constants.CAMPIGN_POLL_PARTICIPATE_COUNT_SIZE;
import static com.techuva.blackcard.contus.app.Constants.SAMSUNG_S3;
import static com.techuva.blackcard.contus.app.Constants.USER_POLL_ID_ANSWER_ARRAY;
import static com.techuva.blackcard.contus.app.Constants.USER_POLL_ID_ANSWER_SELECTED_ARRAY;
import static com.techuva.blackcard.contus.app.Constants.USER_POLL_ID_ANSWER_SIZE;
import static com.techuva.blackcard.contus.app.Constants.USER_POLL_ID_SELECTED_SIZE;
...
public class Participate extends Activity implements EndLessListView.EndlessListener {
    ...
    public void init() {
        ...
        //Campaign name
        String campaignName = getIntent().getExtras().getString(CAMPAIGN_NAME);
        campaignDate = getIntent().getExtras().getString(CAMPAIGN_DATE);

        //camapign category
        String camPaignCategory = getIntent().getExtras().getString(CAMPAIGN_CATEGORY);
        //Campaing logo
        String camPaignLogo = getIntent().getExtras().getString(CAMPAIGN_LOGO);
        //Setting the string in prefernce
        MApplication.setString(Participate.this, CAMPAIGN_ID, camPaignId);
        //Setting the string in prefernce
        MApplication.setString(Participate.this, CAMPAIGN_NAME, campaignName);
        //Setting the string in prefernce
        MApplication.setString(Participate.this, CAMPAIGN_CATEGORY, camPaignCategory);
        //Setting the string in prefernce
        MApplication.setString(Participate.this, CAMPAIGN_LOGO, camPaignLogo);
                            ...
                            if (welcomeResponseModel.getSuccess().equals("1")) {
                                ...
                                if (!dataResults.isEmpty()) {
                                    ...
                                    if (Integer.parseInt(getCurrentPage) == 1) {
                                        ...
                                        campaignPollcommentsCount = MApplication.loadArray(participatePoll, campaignPollcommentsCount, CAMPAIGN_POLL_COMMENTS_COUNT, CAMPAIGN_POLL_COMMENTS_SIZE);
                                        //Loading the details from the arraylist
                                        campaignPollLikesUser = MApplication.loadArray(participatePoll, campaignPollLikesUser, CAMPAIGN_POLL_LIKES_USER_ARRAY, CAMPAIGN_POLL_LIKES_USER_SIZE);
                                        //Loading the details from the arraylist
                                        campaignPollLikeCount = MApplication.loadArray(participatePoll, campaignPollLikeCount, CAMPIGN_POLL_LIKES_COUNT_ARRAY, CAMPIGN_POLL_LIKES_COUNT_SIZE);
                                        //Loading the details from the arraylist
                                        campaignPollParticipateCount = MApplication.loadArray(participatePoll, campaignPollParticipateCount, CAMPIGN_POLL_PARTICIPATE_COUNT_ARRAY, CAMPIGN_POLL_PARTICIPATE_COUNT_SIZE);
                                        //Loading the details from the arraylist
                                        campaignPollIdAnwserCheck = MApplication.loadStringArray(participatePoll, campaignPollIdAnwserCheck, USER_POLL_ID_ANSWER_ARRAY, USER_POLL_ID_ANSWER_SIZE);
                                        //Loading the details from the arraylist
                                        campaignPollIdAnwser = MApplication.loadStringArray(participatePoll, campaignPollIdAnwser, USER_POLL_ID_ANSWER_SELECTED_ARRAY, USER_POLL_ID_SELECTED_SIZE);
                                    }
                                    /**
                                    ...
                                    for (int i = 0; dataResults.size() > i; i++) {
                                        ...
                                        //Save in prefernce
                                        MApplication.saveArray(participatePoll, campaignPollcommentsCount, CAMPAIGN_POLL_COMMENTS_COUNT, CAMPAIGN_POLL_COMMENTS_SIZE);
                                        ...
                                        //Save in prefernce
                                        MApplication.saveArray(participatePoll, campaignPollLikesUser, CAMPAIGN_POLL_LIKES_USER_ARRAY, CAMPAIGN_POLL_LIKES_USER_SIZE);
                                        //poll like count
                                        campaignPollLikeCount.add(Integer.valueOf(welcomeResponseModel.getResults().getData().get(i).getCampaignLikesCounts()));
                                        //Save in prefernce
                                        MApplication.saveArray(participatePoll, campaignPollLikeCount, CAMPIGN_POLL_LIKES_COUNT_ARRAY, CAMPIGN_POLL_LIKES_COUNT_SIZE);
                                        //participate count
                                        campaignPollParticipateCount.add(Integer.valueOf(welcomeResponseModel.getResults().getData().get(i).getPollParticipateCount()));
                                        //Save in prefernce
                                        MApplication.saveArray(participatePoll, campaignPollParticipateCount, CAMPIGN_POLL_PARTICIPATE_COUNT_ARRAY, CAMPIGN_POLL_PARTICIPATE_COUNT_SIZE);
                                        for (int j = 0; welcomeResponseModel.getResults().getData().get(i).getUserParticipatePolls().size() > j; j++) {
                                            if (welcomeResponseModel.getResults().getData().get(i).getUserParticipatePolls().get(j).getUserId().equals(userId)) {
                                                if (welcomeResponseModel.getResults().getData().get(i).getUserParticipatePolls().get(j).getUserId().equals(userId)) {
                                                    ...
                                                    MApplication.saveStringArray(participatePoll, campaignPollIdAnwserCheck, CAMPAIGN_POLL_ID_ANSWER_ARRAY, CAMPAIGN_POLL_ID_ANSWER_SIZE);
                                                    //Save the string array in prefernce
                                                    MApplication.saveStringArray(participatePoll, campaignPollIdAnwser, CAMPAIGN_POLL_ID_ANSWER_SELECTED_ARRAY, CAMPAIGN_POLL_ID_SELECTED_SIZE);
                                                }
                                            }
                                        }
                                    }
