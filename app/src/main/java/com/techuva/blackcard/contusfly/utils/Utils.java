 ...
 */
package com.techuva.blackcard.contusfly.utils;
...
import com.techuva.blackcard.contusfly.emoji.Emojicon;
import com.techuva.blackcard.contusfly.emoji.EmojiconGridView;
import com.techuva.blackcard.contusfly.emoji.EmojiconsPopup;
import com.techuva.blackcard.contusfly.model.ChatMsg;
import com.techuva.blackcard.contusfly.model.Contact;
import com.techuva.blackcard.contusfly.model.MDatabaseHelper;
import com.techuva.blackcard.contusfly.model.PendingNotification;
import com.techuva.blackcard.contusfly.service.ChatService;
import com.techuva.blackcard.contusfly.views.CustomTextView;
...
import static com.techuva.blackcard.contusfly.utils.Constants.PHONE_NUMBER_CODE;
...
public class Utils {
    ...
    public static void loadImageWithGlideProfileRounderCorner(Context context, String imgUrl, ImageView imgView, int errorImg) {
        ...
        if (imgUrl != null && !imgUrl.isEmpty()) {
            ...
            Glide.with(context).load(Uri.parse(imgUrl.replaceAll(" ", "%20"))).diskCacheStrategy(DiskCacheStrategy.ALL).override(80, 100).error(errorImg)
                    .fitCenter()
                    //.bitmapTransform(new RoundedCornersTransformation(context, 20, 50, 0, RoundedCornersTransformation.CornerType.ALL))
    ...
    public static void loadImageWithGlideRounderCorner(Context context, String imgUrl,
                                                       ImageView imgView, int errorImg) {
        if (imgUrl != null && !imgUrl.isEmpty())
            Glide.with(context).load(Uri.parse(imgUrl.replaceAll(" ", "%20"))).diskCacheStrategy(DiskCacheStrategy.ALL).error(errorImg)
                    .fitCenter()
                    //.bitmapTransform(new RoundedCornersTransformation(context, 10, 20, 0, RoundedCornersTransformation.CornerType.ALL))
                    .diskCacheStrategy(DiskCacheStrategy.RESOURCE)
                    .dontAnimate()
                    .placeholder(errorImg).into(imgView);
        else
    ...
    public static void loadImageWithGlideSingleImageRounderCorner(Context context, String imgUrl,
        ...
        if (imgUrl != null && !imgUrl.isEmpty())
            Glide.with(context).load(Uri.parse(imgUrl.replaceAll(" ", "%20"))).diskCacheStrategy(DiskCacheStrategy.ALL).error(errorImg)
                    ...
                    .fitCenter()
                    //.bitmapTransform(new RoundedCornersTransformation(context, 20, 60, 0, RoundedCornersTransformation.CornerType.ALL))
                    .diskCacheStrategy(DiskCacheStrategy.RESOURCE)
                    .dontAnimate()
                    .placeholder(errorImg).into(imgView);
        else
    ...
    private static class EmailTask extends
         ...
         * @param contactName the contact name
         */
        public EmailTask(Context context, String contactName) {
            this.context = context;
    ...
    public static class ContactSync extends AsyncTask<Void, Void, String> {
        ...
        public ContactSync(Context context, String deviceCountryCode) {
            this.context = context;
            phoneCode = MApplication.getString(context, PHONE_NUMBER_CODE);
                    ...
                    if (Integer.parseInt(
                        ...
                        if (pCur != null) {
                            ...
                            if (CountryZipCode.isEmpty())
                                countryCode = MApplication.getString(context, PHONE_NUMBER_CODE);
                            if (!CountryZipCode.isEmpty()) {
...
*/
                                        ...
                                        if (mobileNos.startsWith(","))
                                            mobileNos = mobileNos.substring(1);
    ...
    public void createNotification(MApplication mApplication, Context context, String title,
        ...
        try {
            ...
            int messageCounts = Integer.parseInt(mApplication.getStringFromPreference(GET_MESSAGE_UNREAD_COUNT)) + 1;
            MApplication.storeStringInPreference(GET_MESSAGE_UNREAD_COUNT, String.valueOf(messageCounts));
